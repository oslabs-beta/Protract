        ...
        <dialog className="modal"  id="signUpModal">
            ...
            <form className="flex flex-col border-0 border-purple-600" action="/signup" method="post">
                ...
                <div className="flex flex-grow justify-between border-0 border-blue-600 my-2">
                    <button className="text-white bg-red-800 border-0 border-red-800 w-1/4 min-w-fit px-2 rounded" type="submit">Sign Up</button>
                    <button className="text-red-800 bg-white border-2 border-white w-1/4 min-w-fit p-2 rounded" onClick={() => (document.querySelector("#signUpModal") as HTMLDialogElement).close()}>Cancel</button>
                </div>
            </form>
        </dialog>
    )
